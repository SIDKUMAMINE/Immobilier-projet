...
import {
  ...
  ArrowLeft, Search, Clock, Pencil, Save, AlertTriangle
} from 'lucide-react';
...
export default function ProprietairesPage() {
  ...
  // ── CRUD propriétaire : état création ──
  const [showCreateForm, setShowCreateForm] = useState(false);
  const [createForm, setCreateForm] = useState({
    first_name: '', last_name: '', phone: '', email: '',
    project_location: '', budget_min: '', budget_max: '',
    priority: 'medium', notes: '',
  });
  const [creating, setCreating] = useState(false);

  // ── CRUD propriétaire : état édition fiche ──
  ...
  // ── CRUD propriétaire : état suppression ──
  ...
  const createLead = async () => {
    ...
    if (error) {
      ...
      setCreateForm({ first_name: '', last_name: '', phone: '', email: '', project_location: '', budget_min: '', budget_max: '', priority: 'medium', notes: '' });
  ...
  // ── UPDATE : modifier la fiche propriétaire ──
  const startEditFiche = () => {
    ...
    setEditForm({
      ...
      project_location: selected.project_location || '',
  ...
  const saveEditFiche = async () => {
    ...
    const patch = {
      ...
      project_location: editForm.project_location.trim() || null,
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={() => setShowCreateForm(true)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 18px', background: `linear-gradient(135deg,${T.gold},${T.goldLight})`, color: T.navy, fontSize: '13px', fontWeight: 600, borderRadius: '9px', border: 'none', cursor: 'pointer' }}>
              <Plus size={14} /> Nouveau propriétaire
            </button>
                      ...
                      <div style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.25)', borderRadius: '10px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                          ...
                          <div style={{ fontSize: '13px', color: T.navy, lineHeight: 1.5 }}>
                            Voulez-vous vraiment supprimer <strong>{displayName(selected)}</strong> ? Cette action supprimera également ses visites et son mandat associés. Cette opération est irréversible.
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    {editingFiche ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                          ...
                          <div style={{ gridColumn: 'span 2' }}><label style={lbl}>Localisation du bien</label><input value={editForm.project_location} onChange={e => setEditForm(f => ({ ...f, project_location: e.target.value }))} style={inp} /></div>
                          <div><label style={lbl}>Prix min souhaité (MAD)</label><input type="number" value={editForm.budget_min} onChange={e => setEditForm(f => ({ ...f, budget_min: e.target.value }))} style={inp} /></div>
                          <div><label style={lbl}>Prix max souhaité (MAD)</label><input type="number" value={editForm.budget_max} onChange={e => setEditForm(f => ({ ...f, budget_max: e.target.value }))} style={inp} /></div>
                      ...
                      <>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                          {[
                            ...
                            <div key={i} style={{ background: T.ivory, borderRadius: '8px', padding: '12px', gridColumn: (f as any).span ? `span ${(f as any).span}` : 'span 1' }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ ...card, width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto', padding: '28px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '24px', fontWeight: 400, color: T.navy, margin: 0 }}>
                Nouveau <span style={{ color: T.gold, fontStyle: 'italic' }}>propriétaire</span>
              </h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div><label style={lbl}>Localisation du bien</label><input value={createForm.project_location} onChange={e => setCreateForm(f => ({ ...f, project_location: e.target.value }))} style={inp} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div><label style={lbl}>Prix min souhaité (MAD)</label><input type="number" value={createForm.budget_min} onChange={e => setCreateForm(f => ({ ...f, budget_min: e.target.value }))} style={inp} /></div>
                <div><label style={lbl}>Prix max souhaité (MAD)</label><input type="number" value={createForm.budget_max} onChange={e => setCreateForm(f => ({ ...f, budget_max: e.target.value }))} style={inp} /></div>
              </div>
              <div>
                <label style={lbl}>Priorité</label>
              ...
              <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                <button onClick={createLead} disabled={creating || !createForm.first_name.trim()} style={{ flex: 1, padding: '11px', background: `linear-gradient(135deg,${T.gold},${T.goldLight})`, color: T.navy, border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', opacity: (!createForm.first_name.trim() || creating) ? 0.6 : 1 }}>
                  {creating ? 'Création...' : 'Créer le propriétaire'}
                </button>
