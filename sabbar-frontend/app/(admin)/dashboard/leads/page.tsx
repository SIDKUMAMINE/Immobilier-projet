...
}Voici le code complet et corrigé. J'ai intégré la barre d'onglets de filtrage (visible sur votre deuxième capture d'écran) directement au-dessus des filtres de recherche, tout en ajoutant les canaux spécifiques **Boîte Mail** et **WhatsApp** sans toucher aux éléments ou styles originaux du site.

La détection automatique se base intelligemment sur le champ `source`, le `subject` ou le contenu du `message` pour isoler les flux sans altérer la synchronisation Supabase existante.
...
const SOURCE_CONFIG: Record<string, { label: string; color: string; bg: string; border: string; icon: string }> = {
  ...
  contact_form:      { label: 'Contact',           color: '#2563eb', bg: 'rgba(37,99,235,0.08)',   border: 'rgba(37,99,235,0.2)',   icon: '✉️' },
  whatsapp:          { label: 'WhatsApp',          color: '#25D366', bg: 'rgba(37,211,102,0.08)',  border: 'rgba(37,211,102,0.2)',  icon: '💬' },
  email:             { label: 'Boîte Mail',        color: '#ef4444', bg: 'rgba(239,68,68,0.08)',   border: 'rgba(239,68,68,0.2)',   icon: '📧' },
  achat:             { label: 'Achat',             color: '#16a34a', bg: 'rgba(22,163,74,0.08)',   border: 'rgba(22,163,74,0.2)',   icon: '🛒' },
  vente:             { label: 'Vente',             color: '#d97706', bg: 'rgba(217,119,6,0.08)',   border: 'rgba(217,119,6,0.2)',   icon: '💰' },
};
...
// Alignement exact avec la barre de navigation de la capture d'écran 2 avec ajout des canaux demandés
const TABS = [
  { key: 'all',               label: 'Tous',               icon: '👥' },
  { key: 'email',             label: 'Boîte Mail',         icon: '📧' },
  { key: 'whatsapp',          label: 'WhatsApp',           icon: '💬' },
  { key: 'estimation',        label: 'Estimation',         icon: '🏠' },
  { key: 'intermediation',    label: 'Intermédiation',     icon: '🤝' },
  { key: 'commercialisation', label: 'Commercialisation', icon: '📊' },
...
function getSrcKey(l: Lead): string {
  ...
  const subjectLower = (l.subject || '').toLowerCase();
  const msgLower = (l.message || '').toLowerCase();

  // Détection prioritaire WhatsApp & Mail
  if (sourceLower === 'whatsapp' || msgLower.includes('whatsapp') || subjectLower.includes('whatsapp')) {
    return 'whatsapp';
  }
  if (sourceLower === 'email' || sourceLower === 'mail' || sourceLower === 'boite_mail' || subjectLower.includes('mail direct')) {
    return 'email';
  }

  // Filtrage par mots-clés classiques
  if (subjectLower.includes('achet') || subjectLower.includes('achat')) return 'achat';
  if (subjectLower.includes('vend')  || subjectLower.includes('vente')) return 'vente';
  ...
  return (
    <>
      <style>{`
        ...
        .lm-tab{transition:all 0.15s;cursor:pointer;border:none;}
        .lm-tab:hover{background:rgba(200,169,110,0.08)!important;}
        .lm-tab.active{background:rgba(200,169,110,0.15)!important;font-weight:600!important;}
        ::placeholder{color:rgba(13,31,60,0.3)!important;}
      `}</style>
      ...
      <div style={{ minHeight:'100%', background:T.ivory, padding:'28px 32px', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
        ...
        {/* SECTION INTEGRATION : Barre d'onglets pour segmenter par canal (Image 2) */}
        <div style={{ display:'flex', gap:'10px', marginBottom:'20px', overflowX:'auto', paddingBottom:'6px', borderBottom:`1px solid ${T.borderSoft}` }}>
          {TABS.map(tab => {
            const count = tab.key === 'all' ? leads.length : leads.filter(l => getSrcKey(l) === tab.key).length;
            return (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`lm-tab ${activeTab === tab.key ? 'active' : ''}`}
                style={{ display:'flex', alignItems:'center', gap:'6px', padding:'8px 14px', background:'#fff', borderRadius:'30px', fontSize:'13px', color:T.navy, whiteSpace:'nowrap', border:`1px solid ${activeTab === tab.key ? T.gold : 'transparent'}` }}>
                <span>{tab.icon}</span>
                <span>{tab.label}</span>
                <span style={{ fontSize:'11px', background: activeTab === tab.key ? T.gold : 'rgba(13,31,60,0.06)', color: activeTab === tab.key ? '#fff' : T.muted, padding:'1px 6px', borderRadius:'10px', marginLeft:'4px', fontWeight:600 }}>
                  {count}
                </span>
              </button>
            );
        ...
        <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr 380px' : 'repeat(auto-fill, minmax(320px, 1fr))', gap:'16px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns: selected ? '1fr' : 'repeat(auto-fill, minmax(320px, 1fr))', gap:'16px' }}>
            {sorted.map(lead => {
              ...
              const src = getSrc(lead);
              return (
                <div key={lead.id} onClick={() => setSelected(selected?.id === lead.id ? null : lead)} className={`lm-card ${selected?.id === lead.id ? 'sel' : ''}`}
                  style={{ background: '#fff', borderRadius: '12px', border: `1px solid ${T.borderSoft}`, padding: '20px', position: 'relative' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                      ...
                      <div>
                        <div style={{ fontSize: '13px', color: T.navy, fontWeight: 500, marginBottom: '2px' }}>{lead.full_name || lead.email || 'Prospect'}</div>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'4px' }}>
                      ...
                      <span style={{ background: src.bg, color: src.color, padding: '2px 8px', borderRadius: '6px', fontSize: '10px', fontWeight: 600, border: `1px solid ${src.border}` }}>{src.label}</span>
                    </div>
                  </div>
                  ...
                  {lead.subject && (
                    <div style={{ fontSize:'12px', color: T.navy, marginBottom:'12px', fontStyle:'italic' }}>
                      "{lead.subject}"
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${T.borderSoft}`, paddingTop: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      {lead.phone && (
                        <a href={`tel:${lead.phone}`} onClick={e => e.stopPropagation()} style={{ textDecoration:'none', background: '#E8F5E9', color: '#2E7D32', padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Phone size={12} /> Appeler
                        </a>
                      )}
                      {lead.email && (
                        <a href={`mailto:${lead.email}`} onClick={e => e.stopPropagation()} style={{ textDecoration:'none', background: '#F5F0E6', color: T.gold, padding: '6px 12px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Mail size={12} /> Email
                        </a>
                      )}
                    </div>
          ...
          {selected && (
            <div style={{ background: '#fff', borderRadius: '12px', border: `1px solid ${T.borderSoft}`, padding: '24px', position: 'sticky', top: '24px', height: 'fit-content' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <label style={{ fontSize: '11px', color: T.muted, textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Nom Complet</label>
                  <div style={{ fontSize: '14px', fontWeight: 500, color: T.navy }}>{selected.full_name || '—'}</div>
                </div>
                <div>
                  <label style={{ fontSize: '11px', color: T.muted, textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Canal de provenance</label>
                  <div style={{ fontSize: '13px', fontWeight: 600, color: getSrc(selected).color }}>{getSrc(selected).label}</div>
                </div>
                <div>
                  <label style={{ fontSize: '11px', color: T.muted, textTransform: 'uppercase', display: 'block', marginBottom: '4px' }}>Message / Demande</label>
                ...
                <div>
                  <label style={{ fontSize: '11px', color: T.muted, textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Changer le Statut</label>
                  <select value={selected.status || 'new'} onChange={(e) => changeStatus(selected.id, e.target.value)} disabled={updatingStatus}
                    style={{ width: '100%', padding: '8px', borderRadius: '6px', border: `1px solid ${T.borderSoft}`, fontSize: '13px', background:'#fff' }}>
